...
## 6. Cambios aplicados (tras aprobación)

Resueltos:
- **B2 — Motor de niveles:** server-side (migración 010, `recalc_level`/`month_compliance`, `GET /levels/me`). ≥70%/mes natural; entrada a mitad de mes no cuenta. Sin escritura cliente. 5 tests.
- **B3 — Comunidades iniciales:** 7 sembradas (migración 011).
- **B4 — Tareas "del día":** fecha del servidor + purga de días anteriores (sin histórico).
- **B5 — Hábitos:** marcar/desmarcar/eliminar/% del mes en UI; fecha de log fijada por el servidor (cierra bypass de fechas pasadas).
- **B8 — Home/Niveles:** leen el nivel desde `/levels/me`, no por la ruta pública.
- **B9 — Auth email/contraseña + recuperación:** implementado (migración 012, bcrypt). 4 tests.

Decisiones aplicadas: fecha del "día" la fija el **servidor**; niveles desde el **mes natural siguiente** al alta; **sin offline** (solo manejo de errores de red).

Pendiente (no bloqueante): B1 (ya no bloquea — hay login por email), B6 (búsqueda parcial), B7 (chat tiempo real), purga de tareas vía cron (hoy es lazy al listar).
